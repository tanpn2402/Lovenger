...
public class SharePhoto extends AppCompatActivity implements PhotoListAdapter.OnEventListener,
    ...
    private void viewPhoto(int currentPhoto){
        Intent viewPhoto = new Intent(this, ActivityViewPhoto.class);
        if(listPhoto.size() > 0){
            viewPhoto.putExtra("photos", listPhoto.toString());
            viewPhoto.putExtra("current", currentPhoto);
        }
    ...
    public void onChildAdded(DataSnapshot dataSnapshot, String s) {
        try {
            ...
            if(!listPhoto.contains(obj.getString("path"))){
                listPhoto.add(obj.getString("path"));
            }
            adapter.add(
                    dataSnapshot.getKey(),
